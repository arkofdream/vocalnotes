...
function scanAllForUsernames() {
    ...
    if (!hintDiv || !userSpan) return;

    let allFoundNames = [];

    // Tüm localStorage'ı tara
...
function backupFullData() {
    try {
        const data = {
            vocalnotes_users: JSON.parse(localStorage.getItem('vocalnotes_users') || '[]'),
            // Sadece aktif kullanıcının verilerini değil, tüm veri setini yedekleyelim (opsiyonel)
            // Ama şimdilik sadece hesaplar ve aktif kullanıcının notları yeterli
        };
        
        // Tüm anahtarları tara ve kullanıcı verilerini ekle
        for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i);
            if (key.startsWith('vocalnotes_data_') || key.startsWith('vocalnotes_reminders_') || key.startsWith('vocalnotes_settings_')) {
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `VocalNotes_FullBackup_${new Date().toISOString().split('T')[0]}.json`;
        a.click();
        URL.revokeObjectURL(url);
        showStatus('✅ Yedek dosyası oluşturuldu!');
    } catch (e) {
        console.error("Backup error:", e);
        showStatus('❌ Yedekleme hatası!', 'warn');
    }
}

function restoreFullData(e) {
    ...
    reader.onload = (event) => {
        try {
            ...
            alert('✅ Veriler başarıyla yüklendi! Uygulama yeniden başlatılıyor...');
            location.reload();
        } catch (err) {
...
document.getElementById('backupBtn').addEventListener('click', backupFullData);
document.getElementById('restoreBtn').addEventListener('click', () => document.getElementById('restoreInput').click());
document.getElementById('restoreInput').addEventListener('change', restoreFullData);
...
window.discoverAllUserData = function() {
    console.log("Deep discovery started...");
    const hintDiv = document.getElementById('registeredUsersHint');
    const userSpan = document.getElementById('userListHint');
    ...
    if (statusDiv) {
        statusDiv.textContent = '🔍 Sistem taranıyor, lütfen bekleyin...';
